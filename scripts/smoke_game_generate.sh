...
curl -fsS "$BASE/v1/game/generate" \
  -H 'Content-Type: application/json' \
  -d '{
    "prompt":"Build a tiny browser arena dodging game with score and upgrades.",
    "goal":"return usable playable scaffold output",
    "improve": true
  }' > "$tmp"
...
assert j.get("feature") == "game", j
assert j.get("type") == "markdown", j

content = j.get("content", "")
files = j.get("files", [])

assert "# Game Design Skeleton" in content, j
...
print("SMOKE_GAME_GENERATE_OK", j.get("feature"), j.get("type"), len(files))
PY
