...
plan = j.get("plan") or {}
assert plan.get("feature") == "game", j
assert isinstance(plan.get("steps"), list) and len(plan["steps"]) >= 4, j

project_run = j.get("project_run") or {}
assert project_run.get("project_id"), j
...
print("SMOKE_BUILDER_RUN_OK", j["builder_target"], scaffold["file_count"])
PY
